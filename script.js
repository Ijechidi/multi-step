...
function checkNum() {
  ...
  } else if (!myNumRegex.test(myNumInput.value)) {
    myNumError.innerHTML = "Must contain numbers only";
  } else {
...
function checkMail() {
  ...
  let myMailError = document.getElementById("errorMail");
  let myMailRegex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;

  if (myMailInput.value.trim() === "") {
    myMailError.innerHTML = "This field is required";
  } else if (!myMailRegex.test(myMailInput.value)) {
    myMailError.innerHTML = "Please enter a valid e-mail";
  } else {
    myMailError.innerHTML = "";
  }
}
